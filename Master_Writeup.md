...
*Master writeup — combined Week 1 to Week 5. Compiled from the committed markdown sources; regenerate with `build_master.py`.*
...
VIX level is the strongest factor (+0.40, t = 4.2): options-implied volatility is the market's own forward-looking dispersion estimate. ΔVIX (+0.12, t = 2.4) and absolute return (+0.05, t = 2.2) are also positive and significant. Drawdown carries a negative coefficient (−0.11, t = −1.8). The drawdown variable is at most zero and equals zero at the trailing peak, so the negative sign means the deeper the market sits below its peak, the higher the forward volatility tends to be — the familiar leverage effect. Holding the other factors fixed, a market at its peak therefore predicts *lower* forward volatility; this is the conditional counterpart to, not a contradiction of, the Section 3 finding that the four shocks happened to erupt from near-peak markets. Rolling excess kurtosis is also negative (−0.04, t = −2.1); once volatility and VIX are controlled for, a kurtosis spike marks a jump that has already happened rather than risk still building. Rolling skewness is indistinguishable from zero.
...
The Section 4 regressions carry two further caveats. The dependent variables are themselves estimates, and the annual fits carry substantial sampling error (especially the tail parameters, fitted on ~250 observations); errors in the dependent variable inflate the residual variance and weaken the apparent relationships, so the flat tail-parameter results are partly a power problem, not only an absence of signal. And with six parameters each tested against the VIX at n = 25, one marginal p-value (NIG α, p = 0.07) is about what chance alone would produce; it should be read as suggestive at most.
